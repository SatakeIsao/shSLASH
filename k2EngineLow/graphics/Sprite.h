...
namespace nsK2EngineLow {
    ...
    const int EXPAND_MAX_SPRITE_EXPAND_SRV = 32;    // 拡張SRVの最大値。
    ...
    struct SpriteInitData {
        ...
        std::array<Texture*, MAX_TEXTURE> m_textures = { nullptr };     // 使用するテクスチャ。DDSファイルのパスが指定されている場合、このパラメータは無視されます。
        const char* m_vsEntryPointFunc = "VSMain";                      // 頂点シェーダーのエントリポイント。
        const char* m_psEntryPoinFunc = "PSMain";                       // ピクセルシェーダーのエントリポイント。
        const char* m_fxFilePath = nullptr;                             // .fxファイルのファイルパス。
        UINT m_width = 0;                                               // スプライトの幅。
        UINT m_height = 0;                                              // スプライトの高さ。
        void* m_expandConstantBuffer = nullptr;                         // ユーザー定義の定数バッファ
        int m_expandConstantBufferSize = 0;                             // ユーザー定義の定数バッファのサイズ。
        std::array<IShaderResource*, EXPAND_MAX_SPRITE_EXPAND_SRV> m_expandShaderResoruceView = { nullptr };            // ユーザー定義のシェーダーリソース。
    ...
    private:
        ...
        ConstantBuffer      m_userExpandConstantBufferGPU;  // ユーザー定義の定数バッファ(GPU用)
        void* m_userExpandConstantBufferCPU = nullptr;      // ユーザー定義の定数バッファ(CPU用)
