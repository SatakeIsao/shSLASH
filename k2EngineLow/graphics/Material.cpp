...
namespace nsK2EngineLow {
	...
	void Material::InitTexture(const TkmFile::SMaterial& tkmMat)
	{
		...
		//アルベドマップ。
		{
			...
			if (albedoMap == nullptr)
			{
				...
				{
					wchar_t wPath[1024] = {};
					mbstowcs_s(nullptr, wPath, filePath, _TRUNCATE);
					albedoMap->InitFromDDSFile(wPath);
				}
		...
		//ノーマルマップ。
		{
			...
			if (normalMap == nullptr)
			{
				normalMap = new Texture();
				{
					wchar_t wPath[1024] = {};
					mbstowcs_s(nullptr, wPath, filePath, _TRUNCATE);
					normalMap->InitFromDDSFile(wPath);
				}
		...
		//スペキュラーマップ。
		{
			...
			if (specularMap == nullptr)
			{
				specularMap = new Texture();
				{
					wchar_t wPath[1024] = {};
					mbstowcs_s(nullptr, wPath, filePath, _TRUNCATE);
					specularMap->InitFromDDSFile(wPath);
				}
		...
		//反射マップ。
		{
			...
			if (reflectionMap == nullptr)
			{
				reflectionMap = new Texture();
				{
					wchar_t wPath[1024] = {};
					mbstowcs_s(nullptr, wPath, filePath, _TRUNCATE);
					reflectionMap->InitFromDDSFile(wPath);
				}
		...
		//屈折マップ。
		{
			...
			if (refractionMap == nullptr)
			{
				refractionMap = new Texture();
				{
					wchar_t wPath[1024] = {};
					mbstowcs_s(nullptr, wPath, filePath, _TRUNCATE);
					refractionMap->InitFromDDSFile(wPath);
				}
				if (!refractionMap->IsValid())
	...
	)
	{
		...
		//比較対象の値がしきい値未満なら0、大きければ1を返す比較関数を設定する。
	...
	void Material::InitPipelineState(
		...
		// 呼び出し間でポインタの値を不変（安定的）にするため static を指定。
		// PSOキャッシュのキーは、このポインタを含む D3D12_GRAPHICS_PIPELINE_STATE_DESC 全体を memcmp で比較する。
		// スタック上のローカル配列にすると呼び出しごとに対象アドレスが変わってしまい、キャッシュが常にミスヒットしてしまう。
