...
namespace nsK2EngineLow
{
    ...
    const Vector3& CharacterController::Execute(const Vector3& targetPosition, float deltaTime)
    {
        ...
        if (isRequestTeleport_) {
            // テレポート処理
            // 衝突計算をスキップして座標をそのまま適用
            position_ = targetPosition;

            // 内部状態のリセット
            verticalVelocity_ = 0.0f; // 落下の勢いなどをリセット
            isOnGround_ = true;       // 安全のため一旦着地状態にする
            isJump_ = false;
        ...
        // 体（Collider）の位置を更新
        // テレポート中も通常時も、最終的に position_ を反映させる
